...
func (e *Engine) ensureWorkspaceAndToolchain(ctx context.Context, ws *state.WorkflowState) error {
	...
	// Seed a .gitignore for Go workspaces so that compiled binaries and test
	// cache artefacts produced by go_build / go_test are never staged by
	// `git add -A` in the checkpoint action.
	if ws.Execution.Workspace != nil && ws.Execution.Toolchain != nil {
		if tcCfg, ok := e.toolchainByID(ws.Execution.Toolchain.ID); ok &&
			toolchainTargetsLanguage(tcCfg, "go") {
			gitignorePath := filepath.Join(ws.Execution.Workspace.Path, ".gitignore")
			if _, statErr := os.Stat(gitignorePath); os.IsNotExist(statErr) {
				const goGitignore = "# Go build outputs — auto-generated by go-orca engine\n" +
					"*.exe\n*.exe~\n*.dll\n*.so\n*.dylib\n" +
					"*.test\n*.out\n" +
					"# Compiled binaries (names without extension)\n" +
					"linear-sync\nserver\nmain\napp\n" +
