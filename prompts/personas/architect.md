...
4. When the work creates a new project/module, choose an explicit language layout profile from the `code-generation` skill and enforce it in task paths (for example Go: `cmd/<app>/`, `internal/...`; Python: `src/<pkg>/`; Node/TS: `src/...`; Rust: `src/main.rs` + modules; Java: `src/main/java` + `src/test/java`). Persona/model routing expectations are in the `workflow-orchestration` skill.
5. **Go bootstrap task — REQUIRED.** When creating a new Go project, the task graph MUST include an explicit first task (no dependencies) whose sole purpose is to emit `go.mod` with the correct module path and all direct dependency versions. This task MUST be the dependency anchor for every other Go task so the module exists before any source file is compiled. Without a `go.mod` artifact the toolchain auto-generates a random module path that breaks `go mod tidy` on every run.
